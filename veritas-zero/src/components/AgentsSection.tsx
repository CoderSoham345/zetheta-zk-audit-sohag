  ...
  <div className="flex items-center gap-3">
    ...
    <div className="flex-1 h-1.5 bg-muted overflow-hidden">
      <motion.div
        ...
        className={color === "green" ? "h-full bg-neon-green" : "h-full bg-neon-purple"}
      />
    </div>
      ...
      <div className="relative z-10 container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {agents.map((agent, i) => (
            ...
            >
              {/* Agent image */}
              <div className={`relative h-72 overflow-hidden ${agent.color === "green" ? "bg-neon-green/5" : "bg-neon-purple/5"}`}>
                ...
                <div className="absolute top-4 left-4">
                  <span className={`tag-label ${agent.color === "green" ? "bg-neon-green/20 text-neon-green" : "bg-neon-purple/20 text-neon-purple"}`}>
                    {agent.role}
                  </span>
                </div>
              </div>
              ...
              <div className="p-6 border-t border-border bg-card/50 backdrop-blur-sm">
                <div className="flex items-baseline justify-between mb-1">
                  <h3 className="font-display text-xl font-black text-foreground group-hover:neon-green-glow transition-all duration-300">
                    {agent.name}
                  </h3>
                ...
                <button className={`mt-6 w-full py-3 font-display text-[10px] font-bold uppercase tracking-[0.2em] border transition-all duration-300 ${
                  agent.color === "green" 
                    ? "border-neon-green/30 text-neon-green hover:bg-neon-green/10 hover:border-neon-green" 
                    : "border-neon-purple/30 text-neon-purple hover:bg-neon-purple/10 hover:border-neon-purple"
                }`}>
                  Select Agent →
                </button>
              </div>
